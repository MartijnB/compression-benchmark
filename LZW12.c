...
{
    unsigned int len = _len(in), bits = 9, next_shift = 512;
    ...
    unsigned int out_len = 0, o_bits = 0;
    uint32_t tmp = 0;
...
int main(int argc, char *argv[]) {
    ...
    void* inpbuffer = _new(char, file_size);

    fread(inpbuffer, 1, (size_t) file_size, f);

    _setsize(inpbuffer, file_size);

    gettimeofday(&time_start, NULL);
    byte *enc = lzw_encode(inpbuffer, 12);
